...
public class SellBinanceBuyBitfinexCommandTest {
    ...
    public void shouldNotKeepOrderWhen_priceGapIsNotProfitable() throws Exception{
        when(bitfinexOrderBookUpdated.getLowestAsk()).thenReturn(new ArbOrderEntry(0.5, 0.2));
        ArbCommand arbCommand = new SellBinanceBuyBitfinexCommand(0, context);
    ...
    public void shouldNotKeepOrderWhen_orderIsMuchHigherThenLowestAsk()throws Exception {
        when(binanceOrderBookUpdated.getLowestAsk()).thenReturn(new ArbOrderEntry(2, 0.2));
        when(bitfinexOrderBookUpdated.getLowestAsk()).thenReturn(new ArbOrderEntry(0.4, 0.2));
        ArbCommand arbCommand = new SellBinanceBuyBitfinexCommand(0, context);
    ...
    public void shouldNotKeepOrderWhen_marketAmountIsSmaller()throws Exception {
        when(binanceOrderBookUpdated.getLowestAsk()).thenReturn(new ArbOrderEntry(2.0, 0.7));
        when(bitfinexOrderBookUpdated.getLowestAsk()).thenReturn(new ArbOrderEntry(0.5, 0.5));
        ArbCommand arbCommand = new SellBinanceBuyBitfinexCommand(0, context);
